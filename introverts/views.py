...
class MealItemsView(APIView):
    ...
    def get(self, request):
        mealitems = MealItems.objects.all()
        mealitems_ser = MealItemsSerializer(mealitems, many=True).data
        return Response({'meals': mealitems_ser}, status=status.HTTP_200_OK)
...
class CategoryItemsView(APIView):
    ...
    def get(self, request):
        ...
        return Response({'categories': categoryitems_ser}, status=status.HTTP_200_OK)
...
class MealsView(APIView):
    ...
    def get(self, request):
        ...
        return Response({'meals': meals_ser}, status=status.HTTP_200_OK)
